...
class ConvertersGenerator
{
    ...
    public function __construct(RouterInterface $router, ConverterListing $converterListing)
    {
        $this->router = $router;
    ...
    public function render(): string
    {
        ...
        foreach ($this->converterListing->list() as $eventWithConverters) {
            $eventsHtml .= $this->buildEventView($eventWithConverters);
        }
...
<div style="width: 100%%; background-color: #eeeeee; padding: 0.5em 0 0.1em 0.3em; font-family: Tahoma, Verdana, sans-serif	">
    <p><a href="%s">< Go back to documentation home</a></p>
</div>
...
<div class="col-lg-8 mx-auto p-3 py-md-5">
    <main>
        <p>These are the event converters registered in the service container. Please remember <strong>this only shows events with at least one converter</strong>.</p>
        <hr class="my-4">
        %s
    </main>
</div>
...
<script src="https://cdn.jsdelivr.net/npm/bootstrap@4.0.0/dist/js/bootstrap.min.js"
        ...
        return \sprintf($html, $this->router->generate('pccomponentes.documentation.home'), $eventsHtml);
    }

    public function isReady(): bool
    {
        return $this->converterListing->hasEvents();
    }
    
    private function buildEventView(EventWithConverters $eventWithConverters): string
    {
        $converterOutput = '';

        foreach ($eventWithConverters->converters() as $converter) {
            $converterOutput .= \sprintf('<li>%s</li>' . \PHP_EOL, $converter);
        }

        return \trim(\sprintf('
<div class="row">
    <div class="col border-bottom mb-4">
        <h3 class="pb-0 mb-0">%s</h3>
        <p class="pt-0 mt-0 text-muted text-monospace"><small>%s</small></p>
        <ul><small><code>%s</code></small></ul>
    </div>
</div>
        ', $eventWithConverters->name(), $eventWithConverters->className(), $converterOutput));
    }
}
